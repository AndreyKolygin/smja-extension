...
- 2025-10-27 — v2.2: DOM chain правила поддерживают несколько групп с именами и флагами активности; подтверждения удаления групп/шагов, адаптивная сетка полей и локализация для новых подсказок.
## 2025-10-27 — v2.3.1
- Импорт стал безопасным: Replace data меняет только группы, присутствующие в JSON; для CVs/System/Output Templates теперь отдельные чекбоксы.
- Добавлены бланки JSON в docs/templates для всех секций (LLM, rules, CV, system, output, integrations).
